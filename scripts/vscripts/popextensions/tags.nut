...
PopExtTags.TagFunctions <- {
	...
	function popext_addcond(bot, args) {
		...
		if ( cond == TF_COND_REPROGRAMMED ) {
			...
			// --- RED SPAWN FALLBACK & COSMETIC LOGIC ---
			// Run with delay to ensure team switch has fully processed and ExtraSpawnPoints logic has finished
			PopExtUtil.RunWithDelay(0.2, function() {
				
				if (!self.IsValid() || !self.IsAlive()) return

				// --- COSMETIC COLOR FIX ---
				// Force all wearables to match the Red team skin (Skin 0)
				for ( local child = self.FirstMoveChild(); child; child = child.NextMovePeer() ) {
					if ( child.GetClassname() == "tf_wearable" ) {
						child.SetTeam( TF_TEAM_PVE_DEFENDERS )
						child.SetSkin( 0 ) // 0 = RED, 1 = BLU
					}
				}

				// --- RED SPAWN FALLBACK LOGIC ---
				// Check if bot is near a BLUE spawn (wrong team!)
				local bot_origin = self.GetOrigin()
				local near_blue_spawn = false
				
				for (local spawn = null; spawn = FindByClassname(spawn, "info_player_teamspawn");) {
					if (spawn && spawn.IsValid() && GetPropInt(spawn, "m_iTeamNum") == TF_TEAM_PVE_INVADERS) {
						if ((spawn.GetOrigin() - bot_origin).Length() < 150.0) {
							near_blue_spawn = true
							break
						}
					}
				}
				
				// If at blue spawn, redirect to red spawn
				if (near_blue_spawn) {
					local red_spawns = []
					for (local spawn = null; spawn = FindByClassname(spawn, "info_player_teamspawn");) {
						if (spawn && spawn.IsValid() && GetPropInt(spawn, "m_iTeamNum") == TF_TEAM_PVE_DEFENDERS) {
							red_spawns.append(spawn.GetOrigin())
						}
					}
					
					if (red_spawns.len() > 0) {
						local new_spawn = red_spawns[RandomInt(0, red_spawns.len() - 1)]
						self.SetOrigin(new_spawn)
						self.SnapEyeAngles(QAngle(0, RandomFloat(0, 360), 0))
						PopExtMain.Error.DebugLog(format("Redirected Reprogrammed bot %s from Blue spawn to Red spawn via AddCond.", self.tostring()))
					}
				}
			}, bot.GetScriptScope())
		}
	...
	function popext_reprogrammed(bot, args) {
        ...
        // Run this with a slight delay to handle cosmetics and spawn fixes
        PopExtUtil.RunWithDelay(0.2, function() {
            ...
            // --- COSMETIC COLOR FIX ---
            // Force all wearables to match the Red team skin (Skin 0)
            // This ensures hats/armor don't stay Blue after the team switch
            for ( local child = self.FirstMoveChild(); child; child = child.NextMovePeer() ) {
                // Check for wearables (hats, miscs, bot armor)
                if ( child.GetClassname() == "tf_wearable" ) {
                    child.SetTeam( TF_TEAM_PVE_DEFENDERS )
                    child.SetSkin( 0 ) // 0 = RED, 1 = BLU
                }
            ...
            if (near_blue_spawn) {
                ...
                if (red_spawns.len() > 0) {
                    ...
                    self.SetOrigin(new_spawn)
                    self.SnapEyeAngles(QAngle(0, RandomFloat(0, 360), 0))
                    PopExtMain.Error.DebugLog(format("Redirected Reprogrammed bot %s from Blue spawn to Red spawn.", self.tostring()))
                }
            }
        }, bot.GetScriptScope())
	}
    ...
    /********************************************************************************************
     * EXTRA SPAWN REDIRECTOR                                                                   *
     *                                                                                          *
     * Accepts the entity/coordinates, spawn_protection variable with time value,               *
	 * and even balanced distribution of multiple robots among several appointed new spawnbots. *
     *                                                                                          *
     * Example: popext_extraspawn{where = `spawnbot_ambush`, spawn_protection = 0 }             *
	 * Above example spawns entity and immediately removes spawn protection.                    *
     *                                                                                          *
     * Example: popext_extraspawn{where = `500 500 500`}                                        *
     ********************************************************************************************/

		function popext_extraspawn(bot, args) {
		local where = "where" in args ? args.where : null
		
		// DEFAULT CHANGED TO 1.0
		// If no argument is provided, the bot gets 1 second of Uber.
		local protection_time = 1.0
		
		if ("spawn_protection" in args) protection_time = args.spawn_protection.tofloat()
		else if ("spawn_uber_duration" in args) protection_time = args.spawn_uber_duration.tofloat()
		else if ("cooldown" in args) protection_time = args.cooldown.tofloat()
		...
		local balanced = "balanced" in args ? args.balanced == "true" : true

		local spawn_point = null
		local spawn_entities = []

		if (where != null && typeof where == "string") {
			for (local spawn = null; spawn = FindByName(spawn, where);) {
				if (spawn && spawn.IsValid()) {
					spawn_entities.append(spawn)
				}
			}

			if (spawn_entities.len() > 0) {
				local selected_spawn = null

				if (balanced) {
					local current_time = Time()
					local num_spawns = spawn_entities.len()
					local num_bots_per_spawn = {}
					
					local needs_reset = false
					foreach (spawn_ent in spawn_entities) {
						local scope = PopExtUtil.GetEntScope(spawn_ent)
						if (!("last_spawn_time" in scope) || (current_time - scope.last_spawn_time) > 0.5) {
							needs_reset = true
							break
						}
					}
					...
					foreach (spawn_ent in spawn_entities) {
						...
						if (needs_reset) {
							scope.bot_count <- 0
							scope.last_spawn_time <- current_time
						}
						num_bots_per_spawn[spawn_ent] <- scope.bot_count
					}
					
					local min_usage = 999
					foreach (spawn_ent, count in num_bots_per_spawn) {
						if (count < min_usage) min_usage = count
					}
					
					local available_spawns = []
					foreach (spawn_ent, count in num_bots_per_spawn) {
						if (count == min_usage) {
							available_spawns.append(spawn_ent)
						}
					}
					
					selected_spawn = available_spawns[RandomInt(0, available_spawns.len() - 1)]
					
					local selected_scope = PopExtUtil.GetEntScope(selected_spawn)
					selected_scope.bot_count <- num_bots_per_spawn[selected_spawn] + 1
					selected_scope.last_spawn_time <- current_time
					
				} else {
					selected_spawn = spawn_entities[RandomInt(0, spawn_entities.len() - 1)]
				}

				if (selected_spawn) {
					local spawn_team = GetPropInt(selected_spawn, "m_iTeamNum")
					local is_reprogrammed = bot.HasBotTag("popext_reprogrammed")
					local bot_expected_team = is_reprogrammed ? TF_TEAM_PVE_DEFENDERS : TF_TEAM_PVE_INVADERS
					
					if (spawn_team > 0 && spawn_team != bot_expected_team) {
						local bot_name = PopExtUtil.GetPlayerName(bot)
						if (bot_name == "" || bot_name == null) bot_name = PopExtUtil.ClassesCaps[bot.GetPlayerClass()]
						
						local team_str_bot = is_reprogrammed ? "RED" : "BLUE"
						local team_str_spawn = spawn_team == 2 ? "RED" : "BLUE"

						ClientPrint(null, HUD_PRINTTALK, format("\x07FFB4B4[POPEXT ERROR]\x07FBECCB Team Mismatch: %s Team Bot '%s' attempted to use %s Team spawn '%s'. Moved to default spawnpoint.", team_str_bot, bot_name, team_str_spawn, where))
						spawn_point = null 
					} 
					else {
						spawn_point = selected_spawn.GetOrigin()
					}
				}
			}
			else {
				local bot_name = PopExtUtil.GetPlayerName(bot)
				if (bot_name == "" || bot_name == null) {
					bot_name = PopExtUtil.ClassesCaps[bot.GetPlayerClass()]
				}
				ClientPrint(null, HUD_PRINTTALK, format("\x07FFB4B4[POPEXT WARNING]\x07FBECCB popext_extraspawn: No spawn points found with targetname '%s' - bot '%s' moved to default spawnpoint.", where, bot_name))
			}
		}
		else if (where != null) {
			spawn_point = PopExtUtil.KVStringToVectorOrQAngle(where, false)
			if (spawn_point == null) {
				local bot_name = PopExtUtil.GetPlayerName(bot)
				if (bot_name == "" || bot_name == null) {
					bot_name = PopExtUtil.ClassesCaps[bot.GetPlayerClass()]
				}
				ClientPrint(null, HUD_PRINTTALK, format("\x07FFB4B4[POPEXT WARNING]\x07FBECCB popext_extraspawn: Invalid coordinates '%s' - bot '%s' moved to default spawnpoint.", where.tostring(), bot_name))
			}
		}

		// === SPAWN PROTECTION LOGIC ===
		if (protection_time > 0.0) {
			bot.AddCondEx(TF_COND_INVULNERABLE_HIDE_UNLESS_DAMAGED, protection_time, null)
		} 
		else if (protection_time == 0.0) {
			bot.RemoveCond(TF_COND_INVULNERABLE)
			bot.RemoveCond(TF_COND_INVULNERABLE_HIDE_UNLESS_DAMAGED)
		}

		if (spawn_point != null) {
			local original_collision = bot.GetCollisionGroup()
			...
			function SpawnHereCollisionFixThink() {
				...
				local dist = (bot.GetOrigin() - spawn_point).Length()
				
				if (dist <= 32.0) {
					bot.SetCollisionGroup(original_collision)
					PopExtUtil.RemoveThink(bot, "SpawnHereCollisionFixThink")
					return
				}
