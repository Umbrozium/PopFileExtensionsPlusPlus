...
function PopExtUtil::DoExplanation( message, print_color = COLOR_YELLOW, message_prefix = "Explanation: ", sync_chat_with_game_text = false, text_print_time = -1, text_scan_time = 0.02, vertical_offset = 0, display_mode = 0, spacing = 1 ) {

	local rgb = HexOrIntToRgb( print_color )

	// Only spawn game_text if mode is 0 (Both) or 1 (Screen Only)
	local txtent = null
	if ( display_mode == 0 || display_mode == 1 ) {
		local y_pos = 0.3 + ( vertical_offset * 0.04 )
		txtent = SpawnEntityFromTable( "game_text", {
			effect = 2,
	...
	// If no txtent exists (Chat Only mode), we need a dummy entity to run the Think
	local think_ent = txtent ? txtent : PopExtUtil.SpawnEnt( "logic_relay", "__popext_chat_think" )

	function ExplanationTextThink() {

		if ( textcooldown > Time() ) return 0.033
		...
		if ( !(i in strarray) ) {
			if ( txtent ) {
				...
				self.Kill() // kill the dummy relay
			}
		...
		if ( text_print_time == -1 || (display_mode == 2 && text_print_time != -1) ) { // auto-duration for chat only mode unless specified
		...
		local should_print_chat = (display_mode == 2) || (display_mode == 0 && sync_chat_with_game_text)
		if (display_mode == 0 && !sync_chat_with_game_text) {
			should_print_chat = false
		}
		
		if (should_print_chat) {
			local final_msg = ""
			if ( message_prefix == null || message_prefix == "" )
				final_msg = format( "\x07%s%s", TF_COLOR_DEFAULT, s )
			else
				final_msg = format( "\x07%s%s \x07%s%s", print_color, message_prefix, TF_COLOR_DEFAULT, s )

			ClientPrint( null, HUD_PRINTTALK, final_msg )

			if ( spacing > 0 ) {
				for ( local k = 0; k < spacing; k++ ) ClientPrint( null, HUD_PRINTTALK, " " )
			}
		}
	...
	AddThink( think_ent, ExplanationTextThink )
}
...
function Info( message, print_color = COLOR_YELLOW, message_prefix = "Explanation: ", sync_chat_with_game_text = false, text_print_time = -1, text_scan_time = 0.02, vertical_offset = 0, display_mode = 0, spacing = 1 ) {
	PopExtUtil.DoExplanation( message, print_color, message_prefix, sync_chat_with_game_text, text_print_time, text_scan_time, vertical_offset, display_mode, spacing )
}

function Explanation( message, print_color = COLOR_YELLOW, message_prefix = "Explanation: ", sync_chat_with_game_text = false, text_print_time = -1, text_scan_time = 0.02, vertical_offset = 0, display_mode = 0, spacing = 1 ) {
	PopExtUtil.DoExplanation( message, print_color, message_prefix, sync_chat_with_game_text, text_print_time, text_scan_time, vertical_offset, display_mode, spacing )
}
