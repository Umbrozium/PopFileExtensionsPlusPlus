...
// ============================================================================
// EXTRA UPGRADES SYSTEM (FINAL - WITH RESET FIX)
// ============================================================================

// ALWAYS reset on script load - this ensures clean state on mission restart
::ExtraUpgradesData <- {
    PlayerState = {},
    Config = null,
    Initialized = false
...
if (!("MenuSystem" in getroottable())) {
    ::MenuSystem <- {
        ...
        ShowMenu = function(player, title, options, callback) {
            if (!player || !player.IsValid()) return
            if (options.len() == 0) {
                PopExtUtil.ShowHudHint("No upgrades available", player, 3.0)
                return
            }
            
            if (player in this.ActiveMenus) {
                this.ActiveMenus[player].closed = true
            }
            
            PopExtUtil.SetPlayerAttributes(player, "move speed bonus", 0.001)
            PopExtUtil.SetPlayerAttributes(player, "no_attack", 1)
            SetPropBool(player, "m_bDrawViewmodel", false)
            ...
            local menu_data = {
                ...
                callback = callback,
                selected = 0,
                last_buttons = GetPropInt(player, "m_nButtons"),
                closed = false,
                last_action = Time()
            }
            
            this.ActiveMenus[player] <- menu_data
            this.RefreshDisplay(player)
            
            local think_name = "MenuThink_" + player.entindex()
            local scope = player.GetScriptScope()
            local self_ref = this
            
            scope[think_name] <- function() {
                if (!(self in self_ref.ActiveMenus)) return
                local m = self_ref.ActiveMenus[self]
                
                if (m.closed) {
                    PopExtUtil.SetPlayerAttributes(self, "move speed bonus", 1.0)
                    return
                }
                
                local btns = GetPropInt(self, "m_nButtons")
                local last = m.last_buttons
                local now = Time()
                local debounce = 0.25
                
                if ((btns & self_ref.IN_RELOAD) && !(last & self_ref.IN_RELOAD)) {
                    m.selected = (m.selected + 1) % m.options.len()
                    self_ref.RefreshDisplay(self)
                }
                if ((btns & self_ref.IN_DUCK) && !(last & self_ref.IN_DUCK)) {
                    m.selected = (m.selected - 1 + m.options.len()) % m.options.len()
                    self_ref.RefreshDisplay(self)
                }
                if ((btns & self_ref.IN_ATTACK) && !(last & self_ref.IN_ATTACK)) {
                    if (now - m.last_action >= debounce) {
                        m.last_action = now
                        m.last_buttons = btns
                        local res = m.callback(self, m.selected, m.options[m.selected])
                        if (!res) self_ref.CloseMenu(self)
                        return 0.1
                    }
                }
                if ((btns & self_ref.IN_ATTACK2) && !(last & self_ref.IN_ATTACK2)) {
                    self_ref.CloseMenu(self)
                    return
                }
                if ((btns & self_ref.IN_SCORE) && !(last & self_ref.IN_SCORE)) {
                    if (now - m.last_action >= debounce) {
                        m.last_action = now
                        self_ref.CloseMenu(self)
                        ::OpenRefundMenu(self)
            ...
            PopExtUtil.AddThink(player, scope[think_name])
        },

        RefreshDisplay = function(player) {
            if (!(player in this.ActiveMenus)) return
            local m = this.ActiveMenus[player]
            
            local text = "=== " + m.title + " ===\n"
            
            local start = 0
            local max_show = 6
            if (m.selected >= max_show) start = m.selected - max_show + 1
            
            for (local i = start; i < m.options.len() && i < start + max_show; i++) {
                if (i == m.selected) text += ">> " + m.options[i] + "\n"
                else text += "   " + m.options[i] + "\n"
            }
            
            if (m.options.len() > max_show) {
                text += format("\n(%d/%d)\n", m.selected + 1, m.options.len())
            }
            
            text += "\n[R] Next [Crouch] Prev [M1] Buy [M2] Exit [TAB] Refund"
            PopExtUtil.ShowHudHint(text, player, 30.0)
        },

        CloseMenu = function(player) {
            if (player in this.ActiveMenus) {
                this.ActiveMenus[player].closed = true
                delete this.ActiveMenus[player]
                PopExtUtil.ShowHudHint("", player, 0.1)
                
                PopExtUtil.SetPlayerAttributes(player, "move speed bonus", 1.0)
                SetPropBool(player, "m_bDrawViewmodel", true)
                PopExtUtil.SetPlayerAttributes(player, "no_attack", 0)
            }
        }
    }
}
...
function GetPlayerCurrency(player) {
    try { return GetPropInt(player, "m_nCurrency") }
    catch(e) { return 0 }
}

function SetPlayerCurrency(player, amount) {
    try { SetPropInt(player, "m_nCurrency", amount) }
    catch(e) {}
}

function ResolveUpgradeValue(template, level, inc) {
    local val = level * inc
    
    local deep_replace = null
    deep_replace = function(obj) {
        if (typeof obj == "string" && obj == "v") return val
        if (typeof obj == "array") {
            local arr = []
            foreach(x in obj) arr.append(deep_replace(x))
            return arr
        }
        return obj
    }
    return deep_replace(template)
}

function CalculateUpgradeCost(upg, current_level) {
    if (typeof upg.cost == "integer") return upg.cost
    if (typeof upg.cost != "array") return upg.cost
    
    local initial = upg.cost[0]
    local inc = upg.cost.len() > 1 ? upg.cost[1] : 0
    local max_cost = upg.cost.len() > 2 ? upg.cost[2] : 99999
    local start_lvl = upg.cost.len() > 3 ? upg.cost[3] : 0
    
    local multiplier = current_level >= start_lvl ? (current_level - start_lvl) : 0
    local final_cost = initial + (multiplier * inc)
    if (final_cost > max_cost) final_cost = max_cost
    return final_cost
}

function CalculateTotalSpent(upg, current_level) {
    local total = 0
    for (local i = 0; i < current_level; i++) {
        total += CalculateUpgradeCost(upg, i)
    }
    return total
}

function FindWeaponByKey(player, config_key) {
    for (local slot = 0; slot <= 7; slot++) {
        local weapon = PopExtUtil.GetItemInSlot(player, slot)
        if (!weapon) continue
        
        local wep_class = weapon.GetClassname()
        local wep_idx = 0
        try { wep_idx = PopExtUtil.GetItemIndex(weapon) } catch(e) {}
        
        if (wep_class == config_key) return weapon
        if (wep_class.tolower() == config_key.tolower()) return weapon
        
        if (config_key in getroottable()) {
            local const_val = getroottable()[config_key]
            if (typeof const_val == "integer" && const_val == wep_idx) return weapon
        }
        
        if ("PopExtItems" in getroottable() && config_key in ::PopExtItems) {
            local def = ::PopExtItems[config_key]
            
            if ("id" in def && def.id == wep_idx) return weapon
            
            if ("item_class" in def) {
                if (typeof def.item_class == "array") {
                    foreach(cls in def.item_class) {
                        if (wep_class == cls) return weapon
                    }
                } else if (wep_class == def.item_class) {
                    return weapon
                }
            }
        }
...
function FindUpgradeByKey(key, config) {
    if ("PlayerUpgrades" in config) {
        if ("All" in config.PlayerUpgrades) {
            foreach(id, upg in config.PlayerUpgrades.All) {
                if (key == "player_all_" + id) return upg
            }
        }
        foreach(classname, upgrades in config.PlayerUpgrades) {
            if (classname == "All") continue
            foreach(id, upg in upgrades) {
                if (key == "player_" + classname + "_" + id) return upg
            }
        }
    }
    
    if ("ItemUpgrades" in config) {
        foreach(wep_key, upgrades in config.ItemUpgrades) {
            foreach(id, upg in upgrades) {
                if (key == wep_key + "_" + id) return upg
            }
        }
    }
...
::ResetExtraUpgrades <- function() {
    printl("[ExtraUpgrades] Resetting all player states...")
    ::ExtraUpgradesData.PlayerState <- {}
    
    // Close all menus
    if ("MenuSystem" in getroottable()) {
        foreach(player, menu in ::MenuSystem.ActiveMenus) {
            if (player && player.IsValid()) {
                ::MenuSystem.CloseMenu(player)
            }
        }
        ::MenuSystem.ActiveMenus <- {}
    }
    
    printl("[ExtraUpgrades] Reset complete.")
}
...
::OpenRefundMenu <- function(player) {
    local userid = PopExtUtil.GetPlayerUserID(player)
    
    if (!(userid in ::ExtraUpgradesData.PlayerState)) {
        PopExtUtil.ShowHudHint("No upgrades to refund", player, 3.0)
        ::OpenExtraUpgradesMenu(player)
        return
    }
    
    local state = ::ExtraUpgradesData.PlayerState[userid]
    local config = ::ExtraUpgradesData.Config
    
    local options = []
    local refund_map = []
    local total_refund = 0
    
    foreach(key, level in state) {
        if (level <= 0) continue
        
        local upg = FindUpgradeByKey(key, config)
        if (!upg) continue
        
        local total_spent = CalculateTotalSpent(upg, level)
        local refund_amount = (total_spent * 0.5).tointeger()
        total_refund += refund_amount
        
        options.append(format("%s (Lv%d) - Refund $%d", upg.name, level, refund_amount))
        refund_map.append({key = key, upg = upg, level = level, refund = refund_amount})
    }
    
    if (options.len() > 0) {
        options.insert(0, format(">>> REFUND ALL ($%d) <<<", total_refund))
        refund_map.insert(0, {key = "__all__", refund = total_refund})
    }
    
    options.append("<<< Back to Upgrades >>>")
    refund_map.append({key = "__back__"})
    ...
    local refund_callback = function(p, idx, txt) {
        local entry = refund_map[idx]
        local uid = PopExtUtil.GetPlayerUserID(p)
        
        if (entry.key == "__back__") {
            ::OpenExtraUpgradesMenu(p)
            return false
        }
        
        local st = ::ExtraUpgradesData.PlayerState[uid]
        
        if (entry.key == "__all__") {
            foreach(k, lvl in st) {
                st[k] <- 0
            }
            SetPlayerCurrency(p, GetPlayerCurrency(p) + entry.refund)
            ClientPrint(p, 3, format("Refunded all upgrades: +$%d", entry.refund))
        } else {
            st[entry.key] <- 0
            SetPlayerCurrency(p, GetPlayerCurrency(p) + entry.refund)
            ClientPrint(p, 3, format("Refunded %s: +$%d", entry.upg.name, entry.refund))
        }
    ...
    ::MenuSystem.ShowMenu(player, "REFUND (50%)", options, refund_callback)
}
...
::OpenExtraUpgradesMenu <- function(player) {
    local config = ::ExtraUpgradesData.Config
    if (!config) {
        printl("[ExtraUpgrades] No config!")
        return
    }
    
    local userid = PopExtUtil.GetPlayerUserID(player)
    if (!(userid in ::ExtraUpgradesData.PlayerState)) {
        ::ExtraUpgradesData.PlayerState[userid] <- {}
    }
    local state = ::ExtraUpgradesData.PlayerState[userid]
    ...
    if ("PlayerUpgrades" in config) {
        if ("All" in config.PlayerUpgrades) {
            foreach(i, upg in config.PlayerUpgrades.All) {
                local key = "player_all_" + i
                local lvl = (key in state) ? state[key] : 0
                
                if (lvl < upg.cap) {
                    local cost = CalculateUpgradeCost(upg, lvl)
                    options.append(format("%s ($%d) [%d/%d]", upg.name, cost, lvl, upg.cap))
                    logic_map.append({ 
                        type = "player", 
                        item = null, 
                        item_key = null, 
        ...
        local class_names = ["", "scout", "sniper", "soldier", "demoman", "medic", "heavyweapons", "pyro", "spy", "engineer"]
        local class_idx = player.GetPlayerClass()
        if (class_idx >= 1 && class_idx <= 9) {
            local class_name = class_names[class_idx]
            if (class_name in config.PlayerUpgrades) {
                foreach(i, upg in config.PlayerUpgrades[class_name]) {
                    local key = "player_" + class_name + "_" + i
                    local lvl = (key in state) ? state[key] : 0
    ...
    if ("ItemUpgrades" in config) {
        foreach(wep_key, upgrades in config.ItemUpgrades) {
            local weapon = FindWeaponByKey(player, wep_key)
            if (weapon) {
                foreach(i, upg in upgrades) {
                    local key = wep_key + "_" + i
                    local lvl = (key in state) ? state[key] : 0
                    
                    if (lvl < upg.cap) {
                        local cost = CalculateUpgradeCost(upg, lvl)
                        local display = wep_key.len() > 12 ? wep_key.slice(0, 12) + ".." : wep_key
                        options.append(format("[%s] %s ($%d) [%d/%d]", display, upg.name, cost, lvl, upg.cap))
                        logic_map.append({ 
                            type = "item", 
                            item = weapon, 
                            item_key = wep_key, 
    ...
    if (options.len() == 0) {
        PopExtUtil.ShowHudHint("No Extra Upgrades Available", player, 3.0)
        return
    }
    
    local cb = function(p, idx, txt) {
        if (idx >= logic_map.len()) return true
        
        local entry = logic_map[idx]
        local money = GetPlayerCurrency(p)
        local uid = PopExtUtil.GetPlayerUserID(p)
        local st = ::ExtraUpgradesData.PlayerState[uid]
        
        if (money >= entry.cost) {
            SetPlayerCurrency(p, money - entry.cost)
            
            local upg_key = entry.key
            local new_lvl = (upg_key in st) ? st[upg_key] + 1 : 1
            st[upg_key] <- new_lvl
            
            // Calculate increment value
            local inc_val = 1
            if ("increment" in entry.data) {
                local inc = entry.data.increment
                if (typeof inc == "array" && inc.len() > 1) {
                    inc_val = inc[1]
                    if (typeof inc_val == "string") {
                        try { inc_val = inc_val.tointeger() }
                        catch(e) { try { inc_val = inc_val.tofloat() } catch(e2) {} }
                    }
                } else if (typeof inc == "integer" || typeof inc == "float") {
                    inc_val = inc
                }
            }
            
            local final_val = ResolveUpgradeValue(entry.data.value, new_lvl, inc_val)
            
            // Clean attribute name
            local attr_name = entry.data.attr
            if (typeof attr_name == "string") {
                local split_idx = attr_name.find("|")
                if (split_idx != null) attr_name = attr_name.slice(0, split_idx)
            }
            
            printl("[ExtraUpgrades] ========================================")
            printl("[ExtraUpgrades] Purchasing upgrade: " + entry.data.name)
            printl("[ExtraUpgrades] Attribute: " + attr_name)
            printl("[ExtraUpgrades] New Level: " + new_lvl)
            printl("[ExtraUpgrades] Value: " + final_val)
            printl("[ExtraUpgrades] Item Key: " + entry.item_key)
            printl("[ExtraUpgrades] Item Entity: " + entry.item)
            
            // Build attribute table like ItemAttributes does
            local attrs = {}
            attrs[attr_name] <- final_val
            
            if (entry.type == "item" && entry.item_key) {
                printl("[ExtraUpgrades] Calling SetPlayerAttributesMulti for item: " + entry.item_key)
                PopExtUtil.SetPlayerAttributesMulti(p, entry.item_key, attrs, false, false)
            } else {
                printl("[ExtraUpgrades] Calling SetPlayerAttributes for player")
                PopExtUtil.SetPlayerAttributes(p, attr_name, final_val)
            }
            
            printl("[ExtraUpgrades] ========================================")
            
            ClientPrint(p, 3, format("Purchased: %s (Level %d)", entry.data.name, new_lvl))
            
            ::OpenExtraUpgradesMenu(p)
            return false
        } else {
            ClientPrint(p, 3, format("Not enough! Need $%d, have $%d", entry.cost, money))
            return true
        }
    }
    
    ::MenuSystem.ShowMenu(player, "EXTRA UPGRADES", options, cb)
}

// ============================================================================
// EXTRA UPGRADES SYSTEM (FINAL - FIXED)
// ============================================================================
    ...
    ::MenuSystem <- {
        ...
        ShowMenu = function(player, title, options, callback) {
            ...
            scope[think_name] <- function() {
                ...
                if ((btns & self_ref.IN_ATTACK) && !(last & self_ref.IN_ATTACK)) {
                    if (now - m.last_action >= debounce && m.options.len() > 0) {
                        m.last_action = now
                        m.last_buttons = btns
                        local res = m.callback(self, m.selected, m.options[m.selected])
                        if (!res) self_ref.CloseMenu(self)
                ...
                // TAB - Instant refund all
                if ((btns & self_ref.IN_SCORE) && !(last & self_ref.IN_SCORE)) {
                    if (now - m.last_action >= debounce) {
                        m.last_action = now
                        ::RefundAllUpgrades(self)
                        self_ref.RefreshDisplay(self)
                    }
                }
        ...
        RefreshDisplay = function(player) {
            ...
            text += "\n[R] Next [Crouch] Prev [M1] Buy [M2] Exit [TAB] Refund All"
            PopExtUtil.ShowHudHint(text, player, 30.0)
        },
...
::RefundAllUpgrades <- function(player) {
    local userid = PopExtUtil.GetPlayerUserID(player)
    
    if (!(userid in ::ExtraUpgradesData.PlayerState)) {
        ClientPrint(player, 3, "No upgrades to refund!")
        return
    }
    ...
    foreach(key, level in state) {
        ...
        // Full refund
        local spent = CalculateTotalSpent(upg, level)
        total_refund += spent
    }
    ...
    if (total_refund > 0) {
        ...
        SetPlayerCurrency(player, GetPlayerCurrency(player) + total_refund)
        ClientPrint(player, 3, format("Refunded all upgrades: +$%d", total_refund))
        
        // Refresh menu to show available upgrades again
        ::OpenExtraUpgradesMenu(player)
    } else {
        ClientPrint(player, 3, "No upgrades to refund!")
    }
...
::OpenExtraUpgradesMenu <- function(player) {
    ...
    local cb = function(p, idx, txt) {
        ...
        if (money >= entry.cost) {
            ...
            printl("[ExtraUpgrades] Purchasing: " + entry.data.name)
            printl("[ExtraUpgrades] Attribute: " + attr_name)
            printl("[ExtraUpgrades] Level: " + new_lvl)
            printl("[ExtraUpgrades] Value: " + final_val)
...
function ExtraUpgrades(config) {
    printl("[ExtraUpgrades] Initializing - clearing previous state...")
    ::ExtraUpgradesData.PlayerState <- {}
    ::ExtraUpgradesData.Config = config
    ::ExtraUpgradesData.Initialized = true
    
    if ("MenuSystem" in getroottable()) {
        ::MenuSystem.ActiveMenus <- {}
    }
    
    printl("[ExtraUpgrades] State reset complete.")
    
    if ("ItemUpgrades" in config) {
        printl("[ExtraUpgrades] ItemUpgrades keys:")
        foreach(k, v in config.ItemUpgrades) {
            printl("[ExtraUpgrades]   - " + k)
        }
    }
    if ("PlayerUpgrades" in config) {
        printl("[ExtraUpgrades] PlayerUpgrades keys:")
        foreach(k, v in config.PlayerUpgrades) {
            printl("[ExtraUpgrades]   - " + k)
        }
    }
    
    local station = null
    while (station = Entities.FindByClassname(station, "func_upgradestation")) {
        printl("[ExtraUpgrades] Found upgrade station: " + station)
        
        station.ValidateScriptScope()
        local scope = station.GetScriptScope()
        
        local st_origin = station.GetOrigin()
        local st_mins = station.GetBoundingMins()
        local st_maxs = station.GetBoundingMaxs()
        
        scope.bounds_min <- st_origin + st_mins - Vector(96, 96, 0)
        scope.bounds_max <- st_origin + st_maxs + Vector(96, 96, 144)
        scope.player_inside <- {}
        scope.last_check <- {}
        
        scope.ScanThink <- function() {
            if (!::ExtraUpgradesData.Initialized) return
            ...
            for (local i = 1; i <= MaxClients(); i++) {
                ...
                if (GetPropInt(player, "m_iTeamNum") != 2) continue
                
                local p_idx = player.entindex()
                local pos = player.GetOrigin() + Vector(0, 0, 40)
                
                local inside = (pos.x >= this.bounds_min.x && pos.x <= this.bounds_max.x &&
                               pos.y >= this.bounds_min.y && pos.y <= this.bounds_max.y &&
                               pos.z >= this.bounds_min.z && pos.z <= this.bounds_max.z)
                
                local was_inside = (p_idx in this.player_inside) ? this.player_inside[p_idx] : false
                local last_time = (p_idx in this.last_check) ? this.last_check[p_idx] : 0
                
                if (inside != was_inside && (now - last_time >= cooldown)) {
                    this.player_inside[p_idx] <- inside
                    this.last_check[p_idx] <- now
                    
                    player.ValidateScriptScope()
                    local p_scope = player.GetScriptScope()
                    
                    if (inside) {
                        p_scope.InExtraShop <- true
                        printl("[ExtraUpgrades] Player entered")
                        ::OpenExtraUpgradesMenu(player)
                    } else {
                        p_scope.InExtraShop <- false
                        printl("[ExtraUpgrades] Player left")
                        ::MenuSystem.CloseMenu(player)
                    }
                }
            }
        ...
        AddThinkToEnt(station, "ScanThink")
    }
    
    printl("[ExtraUpgrades] Setup complete!")
}
